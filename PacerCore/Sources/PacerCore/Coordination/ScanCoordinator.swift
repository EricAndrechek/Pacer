...
public final class ScanCoordinator {
    ...
    /// Bump to re-run the one-time single-account backfill.
    public static let currentAccountBackfillVersion = "1"
    ...
    /// Watches which account is logged in and records the activation
    ...
    private func runScanCycle() async throws -> ScanReport {
        ...
        // without guessing. Anyone with two accounts gets nothing here —
        // their history stays unattributed until they say which range was
        // which, because for them there is no sound inference to make.
        let storedAccountBackfill = try fetchMeta(ClaudeCodeMetaKey.accountBackfillVersion)
        if storedAccountBackfill != Self.currentAccountBackfillVersion {
            if let filled = try AccountBackfill.backfillIfUnambiguous(context: context) {
                log("accounts: backfilled \(filled.samplesAttributed) sample(s) to the only known account")
                recorder.invalidate()
                activePersister.accountTrail = recorder.trail()
            }
            try writeMeta(ClaudeCodeMetaKey.accountBackfillVersion,
