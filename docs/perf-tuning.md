...
1. **Phase-timed log lines.** Every `[ScanCoordinator] scan: ...` line in
   ...
   pollution (recovery / cost-version bump / migration / a live check:
   a bucket rebuilt from its samples after a write, a close or the sweep,
   logged as `verify: rebuilding daily a project b hourly c`, #151). A
   ratio that stays near `0/M` cycle after cycle is a
